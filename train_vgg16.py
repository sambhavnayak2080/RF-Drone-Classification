...
def train_model() -> Dict:
    ...
    model = VGG16MultiTask(pretrained=True)
    if num_gpus > 1:
...
def evaluate_per_snr(model_state):
    ...
    model = VGG16MultiTask(pretrained=False)
...
checkpoint_path = f'vgg16_results/{MODEL_NAME}_best.pth'
torch.save({
    'model_name': MODEL_NAME,
...
with open('vgg16_results/results.json', 'w') as f:
    json.dump(results_json, f, indent=2)
print("Saved: vgg16_results/results.json")

zip_filename = 'vgg16_training_results'
shutil.make_archive(zip_filename, 'zip', 'vgg16_results')
